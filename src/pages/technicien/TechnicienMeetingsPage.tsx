...
import { ChevronLeft, ChevronRight, Plus, Video, Trash2, CalendarDays } from 'lucide-react'
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
      <div className="flex items-center justify-between mb-5">
        <button onClick={() => setCurrent(new Date(year, month - 1, 1))} className="p-1.5 rounded-lg hover:bg-slate-100 text-text-muted hover:text-text transition-colors">
          ...
          <div key={i} className="flex items-center justify-center py-1">
            {d ? (
              <div className={`w-8 h-8 flex items-center justify-center rounded-full text-sm font-medium transition-all ${
                ...
                !isToday(d) && !hasMeeting(d) ? 'text-text hover:bg-slate-100' : ''
              }`}>{d}</div>
            ) : null}
          </div>
        ))}
...
export default function TechnicienMeetingsPage() {
  ...
  const { data: meetings } = useCollection<Meeting>('meetings', [
    where('invitedUids', 'array-contains', profile?.uid ?? ''),
    orderBy('date', 'asc'),
  ])
  ...
  return (
    <>
      <Header title="Réunions" subtitle="Planning et accès visio" />
      <div className="p-4 md:p-8">
        ...
        {meetings.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-gray-200 bg-white p-8 flex flex-col items-center text-center">
            <div className="w-16 h-16 rounded-2xl bg-sky-50 flex items-center justify-center mb-4">
              <CalendarDays className="w-8 h-8 text-accent" />
            </div>
            <h3 className="font-semibold text-text">Aucune réunion planifiée</h3>
            <p className="text-sm text-text-muted mt-1">Votre calendrier apparaîtra ici dès qu’une réunion sera créée.</p>
                    ...
                    {upcoming.map((m) => (
                      <Card key={m.id} className="flex items-center justify-between hover:shadow-md transition-shadow">
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => void deleteDoc(doc(db, 'meetings', m.id))} className="p-2 rounded-lg hover:bg-danger/10 text-text-muted hover:text-danger transition-colors">
                  ...
                  <div className="space-y-2">
                    {past.map((m) => (
                      <div key={m.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl border border-gray-100 opacity-60">
                        ...
                        <button onClick={() => void deleteDoc(doc(db, 'meetings', m.id))} className="p-1.5 rounded-lg hover:bg-danger/10 text-text-muted hover:text-danger transition-colors">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {users.filter((u) => u.role === 'technicien' && u.status === 'active').map((t) => {
